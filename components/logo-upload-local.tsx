...
import { Upload, X, Camera, Loader2, CheckCircle, AlertCircle, Info } from "lucide-react"
...
import {
  ...
  type UploadResult,
  type UploadProgress,
} from "@/lib/supabase/storage-local"
...
export function LogoUpload({
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      if (result.success && result.url) {
        ...
        // Verificar se foi salvo no banco ou apenas como base64
        if (result.url.startsWith("data:")) {
          setShowWarning(true)
          toast({
            title: "✅ Logo carregada!",
            description: "Logo salva temporariamente. Execute o script SQL para persistir no banco.",
          })
        } else {
          toast({
  ...
  const handleRemoveLogo = async () => {
    ...
    try {
      const success = await deleteAgenciaLogo(userId)
      if (success) {
        setPreviewUrl(null)
        onLogoChange(null)
        setShowWarning(false)
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {/* Aviso sobre tabela não existir */}
      {showWarning && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
          <div className="flex items-start gap-2">
            <Info className="h-4 w-4 text-amber-600 mt-0.5 flex-shrink-0" />
            <div className="text-sm text-amber-800">
              <p className="font-medium mb-1">Logo salva temporariamente</p>
              <p className="text-xs text-amber-700">
                Execute o script SQL <code>create-simple-tables.sql</code> para persistir a logo no banco de dados.
              </p>
            </div>
          </div>
        </div>
